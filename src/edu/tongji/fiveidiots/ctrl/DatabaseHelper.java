...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public ContentValues seperateTask(TaskInfo aTask){
		int id,  starttime, deadline, alarm;
		String  tag; 
		ArrayList<String> tagArrayList = aTask.ExportTag();
		if (tagArrayList.size() == 0) tag = null;
		...
		starttime = start.getYear() << 20 + start.getMonth() << 16 + start.getDate() << 11 + start.getHours() << 6 + start.getMinutes();
		deadline = dead.getYear() << 20 + dead.getMonth() << 16 + dead.getDate() << 11 + dead.getHours() << 6 + dead.getMinutes();
		alarm = alarmDate.getYear() << 20 + alarmDate.getMonth() << 16 + alarmDate.getDate() << 11 + alarmDate.getHours() << 6 + alarmDate.getMinutes();
	...
	public TaskInfo combineTask(Cursor cursor){
		Date starttime = CombineDate(cursor.getInt(5));
		Date deadline = CombineDate(cursor.getInt(6));
		Date alarm = CombineDate(cursor.getInt(7));
